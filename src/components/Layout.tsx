...
export function Layout({ children, pageTitle }: LayoutProps) {
  return (
    <CelebrationProvider>
      <SidebarProvider>
        <SkipToContent />
        <div className="flex min-h-screen min-h-screen-safe w-full max-w-full overflow-x-hidden">
          <AppSidebar />
          <SidebarInset className="flex-1 flex flex-col min-w-0 max-w-full overflow-x-hidden">
            <MobileHeader />
            <main
              id="main-content"
              className="flex-1 overflow-y-auto overflow-x-hidden min-w-0 max-w-full safe-x safe-bottom"
              role="main"
