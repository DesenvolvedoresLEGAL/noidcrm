...
export function ProposalsList({ opportunityId }: ProposalsListProps) {
  ...
  const getStatusBadge = (status: string) => {
    const variants: Record<string, any> = {
      ...
      sent: { variant: 'default', label: 'Aberta', color: 'bg-blue-100 text-blue-800' },
      viewed: { variant: 'outline', label: 'Aberta · Visualizada', color: 'bg-yellow-100 text-yellow-800' },
      accepted: { variant: 'default', label: 'Aceita', color: 'bg-green-100 text-green-800' },
      rejected: { variant: 'destructive', label: 'Recusada', color: 'bg-red-100 text-red-800' },
    };
