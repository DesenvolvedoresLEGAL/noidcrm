...
interface RepFunnelChartProps {
  data: RepDashboardData["funnelConversion"];
}
...
export function RepFunnelChart({ data }: RepFunnelChartProps) {
  const maxValue = Math.max(data.leads, data.opportunities, data.proposals, data.won, 1);

  return (
    <Card className="h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold">Meu Funil de Conversão</CardTitle>
      </CardHeader>
      <CardContent>
