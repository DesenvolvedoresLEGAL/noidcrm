...
export function OrganizationContractsTab({ organizationId }: OrganizationContractsTabProps) {
  const { data: contracts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch contracts for this specific organization with account data
  ...
  const totalValue = contracts?.reduce((sum, c) => sum + (Number(c.contract_value) || 0), 0) || 0;
              ...
              {!contracts?.length && (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                    Nenhum contrato encontrado para esta organização
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
