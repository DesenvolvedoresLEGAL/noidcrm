...
export function useUnifiedInbox() {
  ...
  const items: InboxItem[] = useMemo(() => {
    ...
    for (const n of v2Query.data ?? []) {
      // hide snoozed items
      if (n.snoozed_until && new Date(n.snoozed_until).getTime() > now) continue;
      list.push({
        id: n.id,
        source: 'v2',
        type: n.type,
    ...
    for (const n of v1Query.data ?? []) {
      // skip if there's a v2 mirror to avoid dup (best-effort by title+created within 5s)
      const dup = list.some(
        (i) =>
          i.source === 'v2' &&
          i.title === n.title &&
    ...
    for (const note of newsQuery.data ?? []) {
      ...
      list.push({
        ...
        meta: { version: note.version, is_major: note.is_major },
      });
    }
