...
function normalizeDate(value: any): string | null {
  ...
  if (typeof value === 'string') {
    ...
    // Handle compact Brazilian dates like 17012020 or 170123
    const compactMatch = trimmed.match(/^(\d{6}|\d{8})$/);
    if (compactMatch) {
      const digits = compactMatch[0];
      const isShortYear = digits.length === 6;
      const day = digits.slice(0, 2);
      const month = digits.slice(2, 4);
      const year = isShortYear ? `20${digits.slice(4)}` : digits.slice(4);
      return `${year}-${month}-${day}`;
    }
  ...
  return rows.map(row => {
    ...
    Object.entries(columnMapping).forEach(([fileColumn, crmField]) => {
      ...
      if (crmField === 'telefones') {
        ...
        value = normalized === '' || normalized.toLowerCase() === 'null' || normalized.toLowerCase() === 'undefined'
          ? null
          : normalized;
      }
