...
const REPORT_KEY = "losses_v2";
const VIEW = "v_report_losses_v2";
const COLS = new Set(["organization_id", "consolidated_loss_reason_id"]);
...
serve(async (req) => {
  ...
  else q = q.order("lost_value", { ascending: false });
  ...
  return okResponse({
    ...
    debug: auth.canDebug ? { view: VIEW, durationMs: Date.now() - t0, limit: options.limit, offset: options.offset } : undefined,
  });
});
