// Sprint Scoring 1.2 — process pending opportunity-score recalc queue items.
// Mirrors process-lead-score-queue: scheduled by pg_cron each minute and
// invokable on demand by the frontend after edits/moves to flush quickly.
...
serve(async (req) => {
  ...
  try {
    ...
    let q = supabase
      .from('opportunity_score_recalc_queue')
    ...
    // Dedupe by opportunity within this cycle
    ...
    for (const [opportunityId, group] of byOpp.entries()) {
