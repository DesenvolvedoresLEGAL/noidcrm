...
const REPORT_KEY = "origins_v2";
const VIEW = "v_report_origins_v2";
const COLS = new Set(["organization_id", "origin_name"]);
...
serve(async (req) => {
  ...
  else q = q.order("won_revenue", { ascending: false });
  ...
  const confidence = await computeConfidence(sb, organizationId, { monetary: true });
  return okResponse({
