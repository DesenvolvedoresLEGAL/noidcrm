-- =============================================================
-- SPRINT REL V2.10 — Vendas Realizadas: receita válida x cancelada
-- Acrescenta colunas no final da view (mantém ordem das existentes).
-- =============================================================
...
enriched AS (
  SELECT
    ...
    -- ===== Cancelamento (sinal interno) =====
    (
      c.canonical_kind = 'sales_won'
      AND (c.op_deleted_at IS NOT NULL OR c.op_status = 'lost')
    ) AS is_cancelled_sale,
    CASE
      WHEN c.canonical_kind = 'sales_won' AND c.op_status = 'lost' THEN 'reopened_lost'
      WHEN c.canonical_kind = 'sales_won' AND c.op_deleted_at IS NOT NULL THEN 'operational_removed'
      ELSE NULL
    END::text AS cancellation_reason,
    c.op_deleted_at AS cancelled_at
  FROM canonical c
...
SELECT
  organization_id, opportunity_id, commercial_opportunity_id, operational_opportunity_id,
  accepted_proposal_id, proposal_number, account_id, account_name, nome_fantasia,
  seller_id, seller_name, won_at, accepted_at, approved_at,
  commercial_amount, commercial_amount_source, is_final_approved_value,
  mrr_amount, one_shot_amount, is_recurring, revenue_type, business_unit,
  legacy_opportunity_value, delta_vs_opportunity_value,
  review_required, warnings, revenue_confidence, created_at,
  pipeline_id, pipeline_name, pipeline_type, opportunity_title,
  status, canonical_kind, commercial_status, fulfillment_status, financial_settlement_status,
  -- ===== Novos campos =====
  is_cancelled_sale,
  cancellation_reason,
  cancelled_at,
  commercial_amount AS approved_amount,
  CASE WHEN is_cancelled_sale THEN commercial_amount ELSE 0 END::numeric AS cancelled_amount,
  CASE WHEN is_cancelled_sale THEN 0 ELSE commercial_amount END::numeric AS valid_revenue_amount,
  CASE
    WHEN is_cancelled_sale THEN 0
    WHEN financial_settlement_status = 'settled' THEN commercial_amount
    ELSE 0
  END::numeric AS liquidated_amount,
  CASE WHEN is_cancelled_sale THEN 0 ELSE commercial_amount END::numeric AS commission_eligible_amount,
  -- Rótulos amigáveis (pt-BR)
  CASE
    WHEN is_cancelled_sale AND cancellation_reason = 'reopened_lost' THEN 'Reaberta'
    WHEN is_cancelled_sale THEN 'Cancelada'
    WHEN review_required THEN 'Em revisão'
    WHEN commercial_status = 'won' THEN 'Aprovada'
    WHEN commercial_status = 'accepted' THEN 'Aprovada'
    ELSE 'Aprovada'
  END::text AS sale_status_label,
  CASE fulfillment_status
    WHEN 'completed' THEN 'Entregue'
    WHEN 'active' THEN 'Em execução'
    WHEN 'not_started' THEN 'Pendente'
    WHEN 'cancelled' THEN 'Cancelada'
    WHEN 'removed' THEN 'Removida'
    WHEN 'not_applicable' THEN 'Não aplicável'
    ELSE 'Pendente'
  END::text AS delivery_status_label,
  CASE
    WHEN is_cancelled_sale THEN 'Cancelada'
    WHEN financial_settlement_status = 'settled' THEN 'Liquidada'
    WHEN financial_settlement_status IN ('pending_payment','pending_settlement_decision','pending_cancellation_fee','pending_credit_decision') THEN 'Pendente de Liquidação'
    WHEN financial_settlement_status = 'manual_review' THEN 'Em revisão'
    ELSE 'Não aplicável'
  END::text AS financial_status_label,
  CASE
    WHEN is_cancelled_sale THEN 'Bloqueada'
    WHEN review_required THEN 'Em revisão'
    WHEN financial_settlement_status IN ('pending_payment','pending_settlement_decision','pending_cancellation_fee','pending_credit_decision') THEN 'Pendente'
    ELSE 'Elegível'
  END::text AS commission_status_label,
  CASE
    WHEN review_required OR revenue_confidence = 'manual_review' THEN 'Revisar'
    WHEN revenue_confidence = 'warning' THEN 'Atenção'
    ELSE 'OK'
  END::text AS audit_status_label
FROM enriched;

COMMENT ON VIEW public.commercial_won_revenue_view IS
  'SSoT P0 — venda comercial. Inclui split de receita válida x cancelada (SPRINT REL V2.10).';

-- =====================================================================
-- commission_eligibility_view: bloqueia venda cancelada explicitamente
-- =====================================================================
...
SELECT
  ...
  CASE
    WHEN v.is_cancelled_sale THEN 'blocked_cancelled'
...
COMMENT ON VIEW public.commission_eligibility_view IS
  'commission_status: blocked_cancelled (SPRINT REL V2.10), blocked_settlement_pending, blocked_review_required, eligible.';
