...
-- Drop old function and recreate as JSONB bundle
DROP FUNCTION IF EXISTS public.get_proposal_by_public_token(text);
...
BEGIN
  -- Find proposal by token (raw or SHA-256 hash)
  ...
  -- Get opportunity's account_id and contact_id
  ...
  v_result := jsonb_build_object(
    ...
    'items', COALESCE((
      ...
      FROM proposal_items i WHERE i.proposal_id = v_proposal.id
    ), '[]'::jsonb),
    ...
    'layout', (
      SELECT jsonb_build_object(
        ...
        'pages', COALESCE((
          SELECT jsonb_agg(to_jsonb(lp) ORDER BY lp.order_index)
