...
AS $$
DECLARE
  v_qual_pipeline_id uuid;
  v_sales_pipeline_id uuid;
  v_loss_matrix jsonb := '[
    -- PRE VENDAS (disqualification)
...
BEGIN
  ...
  FROM pipelines
  WHERE organization_id = p_org_id AND pipeline_type = 'qualification'
  ORDER BY created_at ASC LIMIT 1;

  SELECT id INTO v_sales_pipeline_id
  FROM pipelines
  WHERE organization_id = p_org_id AND pipeline_type = 'sales'
  ORDER BY created_at ASC LIMIT 1;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_loss_matrix) LOOP
    ...
    ELSIF v_item->>'scope' = 'sales' THEN
      ...
      IF v_target_pipeline IS NOT NULL THEN v_target_ids := ARRAY[v_target_pipeline]; END IF;
    ELSE
      v_target_ids := NULL; -- todos os funis
    END IF;
    ...
    IF v_existing.id IS NULL THEN
      -- Não cria automaticamente exceto motivos essenciais de PRE VENDAS
      ...
      END IF;
    ELSE
      -- UPDATE existing reason in-place
  ...
  -- ============ AUDIT: motivos ainda em "todos os funis" ============
  ...
  SELECT jsonb_build_object(
    'qual', COUNT(*) FILTER (WHERE v_qual_pipeline_id = ANY(COALESCE(pipeline_ids, ARRAY[]::uuid[]))),
    'sales', COUNT(*) FILTER (WHERE v_sales_pipeline_id = ANY(COALESCE(pipeline_ids, ARRAY[]::uuid[]))),
