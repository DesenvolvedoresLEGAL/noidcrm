...
BEGIN
  ...
  WHERE pipeline_id = v_pipeline_id ORDER BY order_index ASC LIMIT 1;
  IF v_stage_id IS NULL THEN
    RAISE EXCEPTION 'No stages found for pipeline %', v_pipeline_id;
  END IF;

  SELECT priority_score, reasoning INTO v_priority_score, v_reasoning
  ...
  IF v_prospect.cnpj IS NOT NULL THEN
    ...
    WHERE organization_id = v_org_id AND cnpj = v_prospect.cnpj AND deleted_at IS NULL LIMIT 1;
  END IF;
  ...
  IF v_account_id IS NULL THEN
    INSERT INTO accounts (
      ...
      v_prospect.cnpj,
      CASE WHEN v_prospect.normalized_domain IS NOT NULL THEN 'https://' || v_prospect.normalized_domain ELSE v_prospect.website END,
  ...
  IF v_has_profile THEN
    ...
    IF jsonb_typeof(v_profile.geographic_presence) = 'array' AND jsonb_array_length(v_profile.geographic_presence) > 0 THEN
      SELECT string_agg(value, ', ') INTO v_geographic_presence_text
  ...
  -- Build email payload (DO NOT create activity anymore — frontend will dispatch via SMTP)
