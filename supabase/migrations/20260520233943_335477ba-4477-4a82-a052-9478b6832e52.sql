...
DECLARE
  ...
  v_amounts numeric[];
  v_a numeric; v_b numeric; v_i integer; v_j integer;
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_payment_schedule_total IS NOT NULL AND v_payment_schedule_total > 0 THEN
      v_canonical := v_payment_schedule_total; v_canonical_source := 'payment_schedule';
  ...
  UPDATE public.proposal_financial_audit_runs
     SET status='completed', total_proposals=v_total, ok_count=v_ok,
         divergent_count=v_div_count, total_approved_amount=v_total_approved,
         total_detected_delta=v_total_delta, finished_at=now()
   WHERE id=v_run_id;
