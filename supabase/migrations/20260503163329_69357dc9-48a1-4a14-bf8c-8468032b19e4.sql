CREATE OR REPLACE FUNCTION public.get_nrhs_analytics(p_org_id uuid, p_owner_id uuid DEFAULT NULL::uuid, p_only_privileged boolean DEFAULT true, p_caller_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  FROM base
  LEFT JOIN LATERAL (
    SELECT tier, count(*)::int AS count, COALESCE(sum(b2.value), 0)::numeric AS value
    FROM (
      SELECT
        CASE
          WHEN COALESCE(b3.nrhs_score, 0) >= 90 THEN 'elite'
          WHEN COALESCE(b3.nrhs_score, 0) >= 75 THEN 'healthy'
          WHEN COALESCE(b3.nrhs_score, 0) >= 60 THEN 'risk'
          WHEN COALESCE(b3.nrhs_score, 0) >= 40 THEN 'critical'
          ELSE 'insalubrious'
        END AS tier,
        b3.value
      FROM base b3
