-- Global proposal search RPC: searches across proposal fields, opportunity, account, contacts and items.
-- Respects RLS (SECURITY INVOKER). Used by Comercial > Propostas as a central history.

CREATE INDEX IF NOT EXISTS idx_proposals_title_trgm ON public.proposals USING gin (title gin_trgm_ops) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_proposals_client_name_trgm ON public.proposals USING gin (client_name gin_trgm_ops) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_proposals_proposal_number ON public.proposals (proposal_number) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_proposal_items_name_trgm ON public.proposal_items USING gin (name gin_trgm_ops);
...
AS $$
DECLARE
  q text := NULLIF(trim(coalesce(_q,'')), '');
  qn text;
  pat text;
BEGIN
  ...
  WITH base AS (
    ...
    WHERE p.deleted_at IS NULL
      ...
      AND (
        ...
        OR unaccent(lower(COALESCE(p.content,''))) LIKE pat
...
GRANT EXECUTE ON FUNCTION public.search_proposals_global(text,text,uuid,numeric,numeric,int,date,date,int,int) TO authenticated;