...
BEGIN
  ...
  FOR v_proposal IN
    SELECT p.id, p.proposal_number, p.organization_id, p.opportunity_id,
           p.approval_snapshot, p.approved_amount,
           o.valor_previsto AS opp_value, o.id AS opp_id
  ...
  LOOP
    ...
    IF v_intent_expected IS NOT NULL THEN v_amounts := v_amounts || v_intent_expected; END IF;
