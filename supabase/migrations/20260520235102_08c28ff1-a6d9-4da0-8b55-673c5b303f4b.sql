...
BEGIN
  ...
  UPDATE public.proposal_financial_audit_runs
     SET status='completed', completed_at=now(),
         ...
         total_approved_amount=v_total_approved, total_delta_amount=v_total_delta
   WHERE id=v_run_id;

  RETURN jsonb_build_object(
    'run_id', v_run_id, 'total', v_total, 'ok', v_ok,
    'divergent', v_div_count, 'needs_review', v_review_count,
    'total_approved', v_total_approved, 'total_delta', v_total_delta,
    'dry_run', p_dry_run
  );
END;
$function$;
