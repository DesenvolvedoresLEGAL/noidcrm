-- 1) Coluna para armazenar o cronograma manual
ALTER TABLE public.proposal_payment_terms
  ADD COLUMN IF NOT EXISTS manual_schedule jsonb;

COMMENT ON COLUMN public.proposal_payment_terms.manual_schedule IS
  'Cronograma manual quando payment_condition=custom_schedule. Formato: [{due_date:"YYYY-MM-DD", percent?:number, amount?:number, label?:string}]';

-- 2) Orquestrador: forçar faixa pós-evento quando qualquer parcela vence após event_start_date
...
BEGIN
  ...
  IF v_is_event THEN
    ...
    -- ====== NOVO: detectar maior data de vencimento entre as parcelas ======
    -- 1) manual_schedule (custom_schedule)
    IF v_existing_one_time.payment_condition = 'custom_schedule'
       AND v_existing_one_time.manual_schedule IS NOT NULL
       AND jsonb_typeof(v_existing_one_time.manual_schedule) = 'array' THEN
      FOR v_schedule_entry IN SELECT * FROM jsonb_array_elements(v_existing_one_time.manual_schedule)
      LOOP
    ...
    -- 2) split_50_50 / split_30_70 -> second_payment_due_date
    IF v_existing_one_time.payment_condition IN ('split_50_50','split_30_70')
       AND v_existing_one_time.second_payment_due_date IS NOT NULL THEN
      v_max_due_date := GREATEST(
        COALESCE(v_max_due_date, '1900-01-01'::date),
        v_existing_one_time.second_payment_due_date
      );
    END IF;

    -- 3) parcelado: estimar última parcela = first_installment_date + (installments-1)*intervalo
    IF v_existing_one_time.payment_condition IN ('installments','net_7','net_15','net_30','net_35','invoiced')
       AND v_existing_one_time.first_installment_date IS NOT NULL THEN
      v_max_due_date := GREATEST(
        COALESCE(v_max_due_date, '1900-01-01'::date),
        v_existing_one_time.first_installment_date
          + (GREATEST(COALESCE(v_existing_one_time.installments,1),1) - 1)
            * COALESCE(v_existing_one_time.installment_interval_days, 30)
      );
    END IF;

    -- Resolver event_start_date a partir da regra de preço dinâmico
    SELECT event_start_date INTO v_event_start
      FROM public.proposal_dynamic_pricing_rules
      WHERE proposal_id = p_proposal_id
      LIMIT 1;

    -- Se há parcela após o evento, forçar referência custom para a maior data
    -- (apenas no termo em memória, para que generate_event_antecedence_pricing_for_proposal
    --  já receba o snapshot com a faixa pós-evento)
    ...
    -- Gerar tabela dinâmica por antecedência
    BEGIN
