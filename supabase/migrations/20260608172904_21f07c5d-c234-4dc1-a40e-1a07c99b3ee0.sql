...
BEGIN
  ...
  -- FIX (upfront due date anchor):
  -- Para pagamento à vista (upfront, 1 parcela), quando a referência da
  -- tabela dinâmica é uma data manual escolhida pelo usuário
  -- (custom_date / payment_due_date), o vencimento deve ser exatamente
  -- essa data — não o `first_installment_date` legado, que pode estar
  -- defasado em relação ao que o cliente concordou.
  v_upfront_anchor_date := NULL;
  IF v_pt.id IS NOT NULL
     AND COALESCE(v_pt.payment_condition,'upfront') = 'upfront'
     AND v_installments = 1
     AND COALESCE(v_pt.dynamic_pricing_reference_type,'current_date') IN ('custom_date','payment_due_date')
     AND v_pt.dynamic_pricing_reference_date IS NOT NULL THEN
    v_upfront_anchor_date := v_pt.dynamic_pricing_reference_date;
  END IF;
