...
BEGIN
  ...
  IF v_has_brief AND v_brief.probable_pains IS NOT NULL AND array_length(v_brief.probable_pains, 1) > 0 THEN
    v_note_md := v_note_md || '## Dores prováveis' || E'\n';
    v_note_md := v_note_md || '- ' || array_to_string(v_brief.probable_pains, E'\n- ') || E'\n\n';
  END IF;

  IF v_has_brief AND v_brief.value_hypotheses IS NOT NULL AND array_length(v_brief.value_hypotheses, 1) > 0 THEN
  ...
  IF v_has_brief AND v_brief.objection_predictions IS NOT NULL AND array_length(v_brief.objection_predictions, 1) > 0 THEN
