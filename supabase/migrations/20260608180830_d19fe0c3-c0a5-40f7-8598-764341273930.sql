...
BEGIN
  ...
  WHERE proposal_id = p_proposal_id
  ORDER BY (payment_type = 'one_time') DESC,
           (payment_condition = 'custom_schedule') DESC,
           (dynamic_pricing_reference_type IN ('custom_date','payment_due_date')) DESC,
  ...
  v_upfront_anchor_date := NULL;
  IF v_pt.id IS NOT NULL
     AND COALESCE(v_pt.payment_type, 'one_time') = 'one_time'
     AND COALESCE(v_pt.payment_condition,'upfront') = 'upfront'
     AND v_installments = 1
     AND COALESCE(v_pt.dynamic_pricing_reference_type, v_proposal.dynamic_pricing_reference_type, 'current_date') IN ('custom_date','payment_due_date')
     AND COALESCE(v_pt.dynamic_pricing_reference_date::date, v_proposal.dynamic_pricing_reference_date::date) IS NOT NULL THEN
    v_upfront_anchor_date := COALESCE(v_pt.dynamic_pricing_reference_date::date, v_proposal.dynamic_pricing_reference_date::date);
  END IF;

  v_payment_schedule := '[]'::jsonb;
  FOR v_idx IN 1..v_installments LOOP
    v_due_date := COALESCE(v_upfront_anchor_date, v_pt.first_installment_date, CURRENT_DATE)
...
DO $$
DECLARE
  v_proposal_id uuid := '3ee2e4fc-764b-48fc-aaac-c8a1927cbbae';
  v_due_date date := DATE '2026-07-06';
  v_amount numeric;
  v_schedule jsonb;
BEGIN
  SELECT COALESCE(approved_amount, pricing_approval_amount, pricing_effective_amount, total_amount)
    INTO v_amount
  FROM public.proposals
  WHERE id = v_proposal_id;

  IF v_amount IS NULL THEN
    RAISE EXCEPTION 'EVENTARE proposal % not found or has no approved amount', v_proposal_id;
  END IF;
  ...
  PERFORM set_config('app.allow_frozen_overwrite', 'true', true);

  UPDATE public.proposals
     SET approved_payment_schedule = jsonb_build_object('schedule', v_schedule),
         approval_snapshot = jsonb_set(
           jsonb_set(COALESCE(approval_snapshot, '{}'::jsonb), '{payment_schedule}', v_schedule, true),
           '{payment_schedule_total}', to_jsonb(v_amount), true
         ),
         pricing_breakdown_snapshot = jsonb_set(
           jsonb_set(COALESCE(pricing_breakdown_snapshot, '{}'::jsonb), '{payment_schedule}', v_schedule, true),
           '{payment_schedule_total}', to_jsonb(v_amount), true
         )
   WHERE id = v_proposal_id
     AND status = 'accepted';
