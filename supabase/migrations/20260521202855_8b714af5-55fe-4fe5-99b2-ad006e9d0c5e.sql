...
BEGIN
  IF NEW.status = 'accepted' AND (OLD.status IS NULL OR OLD.status != 'accepted') THEN
    ...
    SELECT string_agg(
      COALESCE(ppt.payment_condition, ppt.payment_type, 'condição') || ': ' ||
      COALESCE(ppt.contract_total::text, ppt.monthly_value::text, NEW.payment_expected_amount::text, NEW.total_amount::text, NEW.value::text, '0'),
      E'\n'
    )
