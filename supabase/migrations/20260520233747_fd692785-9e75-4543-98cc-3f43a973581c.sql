...
BEGIN
  ...
  FOR v_proposal IN
    ...
    WHERE p.organization_id = v_org
      ...
      AND COALESCE(p.approved_at, p.accepted_at, p.closed_at, p.updated_at)::date
          BETWEEN p_period_start AND p_period_end
  LOOP
    v_total := v_total + 1;
    v_div := ARRAY[]::text[];

    -- Slack evidence: enum doesn't include 'slack', try cast text fallback safely
    v_slack_amount := NULL;
    ...
    -- Ledger
    v_ledger_effective := NULL;
    v_ledger_erp := NULL;
    BEGIN
      SELECT effective_amount, erp_amount
        INTO v_ledger_effective, v_ledger_erp
    ...
    v_intent := NULL;
    BEGIN
      SELECT expected_amount, pricing_erp_amount
        INTO v_intent
    ...
    v_snap_amount := NULLIF(v_approval_snapshot->>'effective_amount','')::numeric;

    -- Canonical decision
    v_canonical := NULL;
    v_canonical_source := NULL;
    IF v_snap_amount IS NOT NULL THEN
      v_canonical := v_snap_amount;
      v_canonical_source := 'approval_snapshot';
    ELSIF v_proposal.approved_amount IS NOT NULL THEN
      v_canonical := v_proposal.approved_amount;
      v_canonical_source := 'approved_amount';
    ELSIF v_payment_schedule_total IS NOT NULL AND v_payment_schedule_total > 0 THEN
      v_canonical := v_payment_schedule_total;
      v_canonical_source := 'payment_schedule';
    ELSIF v_ledger_effective IS NOT NULL THEN
      v_canonical := v_ledger_effective;
      v_canonical_source := 'ledger';
      v_reconstructed := v_ledger_effective;
    ELSE
      v_canonical_source := 'indeterminate';
    END IF;

    -- Reconstructed diagnostic if no snapshot
    IF v_snap_amount IS NULL AND v_ledger_effective IS NOT NULL THEN
      v_reconstructed := v_ledger_effective;
    END IF;

    -- Max delta across financial signals (exclude Slack/ERP as they are evidence)
    v_amounts := ARRAY[]::numeric[];
    IF v_canonical IS NOT NULL THEN v_amounts := v_amounts || v_canonical; END IF;
    IF v_deal_amount IS NOT NULL THEN v_amounts := v_amounts || v_deal_amount; END IF;
    IF v_ledger_effective IS NOT NULL THEN v_amounts := v_amounts || v_ledger_effective; END IF;
    IF v_ledger_erp IS NOT NULL THEN v_amounts := v_amounts || v_ledger_erp; END IF;
    IF v_payment_schedule_total IS NOT NULL AND v_payment_schedule_total > 0 THEN v_amounts := v_amounts || v_payment_schedule_total; END IF;
    IF v_intent.expected_amount IS NOT NULL THEN v_amounts := v_amounts || v_intent.expected_amount; END IF;
    ...
    IF v_payment_schedule_total IS NOT NULL AND v_payment_schedule_total > 0 AND v_canonical IS NOT NULL AND abs(v_payment_schedule_total - v_canonical) > 0.01 THEN
      v_div := v_div || 'schedule_mismatch';
    END IF;
    IF v_intent.expected_amount IS NOT NULL AND v_canonical IS NOT NULL AND abs(v_intent.expected_amount - v_canonical) > 0.01 THEN
      v_div := v_div || 'payment_intent_mismatch';
    END IF;
    ...
    INSERT INTO public.proposal_financial_audit_items(
      run_id, organization_id, proposal_id, opportunity_id,
      slack_amount, deal_amount, ledger_effective_amount, ledger_erp_amount,
      ...
      recommended_action, audit_status, reconstructed_ledger_amount,
      raw_values
    ) VALUES (
      v_run_id, v_org, v_proposal.id, v_proposal.opportunity_id,
      v_slack_amount, v_deal_amount, v_ledger_effective, v_ledger_erp,
      v_proposal.approved_amount, v_snap_amount, v_payment_schedule_total,
      v_intent.expected_amount, NULLIF(v_erp.payload->>'amount','')::numeric,
      v_canonical, v_canonical_source, v_max_delta, v_div,
      v_action, v_audit_status, v_reconstructed,
      jsonb_build_object(
        'proposal_number', v_proposal.proposal_number,
        'slack_payload', v_slack_payload,
        'erp_payload', v_erp.payload,
        'approval_snapshot', v_approval_snapshot,
        'opportunity_id', v_proposal.opp_id
      )
    );
  END LOOP;
