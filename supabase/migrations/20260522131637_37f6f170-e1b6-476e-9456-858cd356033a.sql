...
BEGIN
  ...
  IF v_dyn_enabled AND v_subtotal_recurring = 0 THEN
    -- FIX: tolerar starts_at NULL (faixa inicial sem limite inferior)
...
-- Backfill defensivo: ignora propostas que falhem em outros guards (ex.: freeze inconsistency)
DO $$
DECLARE
  r RECORD;
  v_ok int := 0;
...
BEGIN
  FOR r IN
    SELECT DISTINCT p.id
      ...
      JOIN public.proposal_dynamic_pricing_tiers t ON t.proposal_id = p.id
     WHERE p.dynamic_pricing_enabled = true
       AND COALESCE(p.price_frozen_on_approval, false) = false
       AND COALESCE(p.status, '') NOT IN ('cancelled','declined')
       AND t.starts_at IS NULL
  LOOP
    BEGIN
      PERFORM public.recalculate_proposal_pricing_ledger(r.id);
      v_ok := v_ok + 1;
    EXCEPTION WHEN OTHERS THEN
      v_fail := v_fail + 1;
      RAISE NOTICE 'Skipping proposal % during backfill: %', r.id, SQLERRM;
    END;
  END LOOP;
  RAISE NOTICE 'Pricing ledger backfill: ok=%, skipped=%', v_ok, v_fail;
END $$;