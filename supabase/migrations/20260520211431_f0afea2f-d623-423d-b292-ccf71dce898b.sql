...
BEGIN
  ...
  -- ---- Subtotal de itens (one_time) e recurring ----
  SELECT
  ...
  IF v_dyn_enabled AND v_subtotal_recurring = 0 THEN
    ...
    IF v_dyn_tier.id IS NOT NULL THEN
      IF v_dyn_tier.adjustment_type IN ('percent','percentage','pct','percent_adjustment') THEN
        v_dyn_pct := COALESCE(v_dyn_tier.adjustment_value, 0);
        v_dyn_amount := ROUND((v_base_amount * v_dyn_pct / 100.0)::numeric, 2);
      ELSIF v_dyn_tier.adjustment_type IN ('amount','flat','absolute','amount_adjustment') THEN
